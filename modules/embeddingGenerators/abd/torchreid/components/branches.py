...
from modules.embeddingGenerators.abd.torchreid.components.attention import get_attention_module_instance
from modules.embeddingGenerators.abd.torchreid.utils.torchtools import init_params
...
class MultiBranchNetwork(nn.Module):
    ...
    def __init__(self, backbone, num_classes, **kwargs):
        ...
        self.common_branch = self._get_common_branch(backbone)
        self.branches = nn.ModuleList(self._get_branches(backbone))

    def _get_common_branch(self, backbone):
        return NotImplemented

    def _get_middle_subbranch_for(self, backbone, last_branch_class):

        return NotImplemented

    def _get_branches(self, backbone) -> list:
        branch_list = []

        middle_subbranch = self._get_middle_subbranch_for(backbone, GlobalBranch)
        global_branch = GlobalBranch(self, backbone, middle_subbranch.out_dim)
        ...
        middle_subbranch = self._get_middle_subbranch_for(backbone, ABDBranch)
        abd_branch = ABDBranch(self, backbone, middle_subbranch.out_dim)
        branch_list.append(
            Sequential(
                middle_subbranch,
                abd_branch
...
class GlobalBranch(nn.Module):
    ...
    def __init__(self, owner, backbone, input_dim):
        super().__init__()
...
class ABDBranch(nn.Module):
    ...
    def _init_attention_modules(self):
        ...
        DAN_module_names = {'cam', 'pam'} & set(['cam', 'pam'])
        use_head = not False
